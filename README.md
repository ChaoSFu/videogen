...
- 填 prompt / 时长 / 分辨率 / seed / 推理步数，点"生成"，H3 是分钟级推理，页面
  会显示已用时长，完成后原地播放结果视频
- Prompt 始终会发送，不管有没有传图片/参考文件；上传的图片/参考文件只决定
  用哪个模式——可选上传首帧/尾帧图片（自动切到 FL2VA）或参考图片/视频/音频
  （自动切到 Ref2VA），都有预览（图片缩略图 / 视频音频文件名 chip）和清除/
  移除按钮，优先级 参考文件 > 首尾帧 > 纯文字（三选一决定模式，不是三选一
  决定要不要用 prompt）
- 页面顶部会显示当前正在进行的任务（不管是谁发起的）；并发点"生成"不会报错，
  会自动排队等轮到自己
- 下方"历史记录"列出过往生成（成功的带视频预览，失败的显示错误信息），刷新
  页面不会丢失；点 prompt 文字可以复制完整内容（卡片上只显示两行截断）；每张
  卡片有"复用"（把 prompt/时长/分辨率/seed/步数填回表单，方便改改再生成——
  fl2va/ref2va 用到的图片/参考文件不会保存，需要重新上传）和"删除"（连同
  生成的视频文件一起删除，不可恢复，会有确认弹窗）
...
`GET /v1/videos?limit=50` 是同一份数据的 API 出口，`DELETE /v1/videos/{id}`
删除一条（同时尝试删除对应的视频文件，文件已经不在了也不会报错），前端和你
自己写脚本都能用。
...
- **Ref2VA**（参考图像/视频/音频 + 文本 → 视频）：P2，已接入。Web 界面上传
  任意参考文件即自动切到 Ref2VA；API 调用时通过 `options.references` 传一个
  列表，每项 `{"data": "<base64 或 data: URL>", "filename": "...", "content_type": "..."}`
